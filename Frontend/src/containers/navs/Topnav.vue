    ...
    <div class="navbar-right">
      <!-- <div class="position-relative d-none d-sm-inline-block">
        ...
        >
          ...
          <div>
            <router-link
              :to="`${adminRoot}/inventario/tipo-producto`"
            ...
            <router-link
              :to="`${adminRoot}/ventas/registro/nuevav`"
              class="icon-menu-item"
            >
              <i class="iconsminds-shopping-cart d-block" />
              {{  }}
            </router-link>
            <router-link :to="`/zapateria/principal`" class="icon-menu-item">
        ...
        >
          <!-- <template slot="button-content">
            <i class="simple-icon-bell" />
               <span v-if="$apollo.data" class="count">{{
              number(getAllNotificaciones)
            }}
            </span>
          </template> -->
          ...
          >
            <!-- <div
              class="d-flex flex-row mb-3 pb-3 border-bottom"
                v-for="(n, index) in getAllNotificaciones"
              :key="index"
            > -->
              <!-- <router-link :to="`${adminRoot}/pages/product/details`"> -->
              <!-- <a href="javascript:void(0);">
              <p @click="leido(n.id)" class="zmdi zmdi-comment-text">
                    {{ n.mensaje }}
              </p>
              </a> -->
              <!-- </router-link> -->
            ...
            <!-- </div>
            <li v-if="number(getAllNotificaciones) < 1">
              <a href="javascript:void(0);">
...
export default {
  apollo:{
        getAllNotificaciones:{
            query:gql`
                query Notificaciones($estado:Boolean!){
                    getAllNotificaciones(estado:$estado){
                        id
                        mensaje
                        estado
                        userId
  ...
  methods: {
    ...
    number(value) {
      if (typeof value != "undefined") return value.length;
    },
    search() {
    ...
    // logout() {
    //   this.signOut().then(() => {
    //     // remover el token, aqui el mensaje pueden meterlo en una notificacion
    //     // con estilo.
    //     let respuesta = Auth.logout()
    ...
    // },
     logout() {
      this.signOut().then(() => {
        ...
        this.$notify("error", "Final de sesión", "Sesion finalizada", {
          duration: 3000,
          permanent: false,
        });
        this.$router.push("/user/login");
